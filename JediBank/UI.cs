...
namespace JediBank
{
    internal class UI
    {
        public string ReadUserName(Language language)
        {
            string usernamePrompt = language.GetUsernamePrompt();
        ...
        public string ReadPassword(Language language)
        {
            Console.CursorVisible = true;
        ...
        public string MainMenu(Dictionary<string, string[]> menuItems, string? message)
        {
            ...
            while (true)
            {
                do
                {
                    ...
                    PaintBox(language.TranslationTool("Main Menu"), width, height, 0, language);
                    //Console.WriteLine("Välkommen " + message.ToUpper());
                    var time = DateTime.Now.TimeOfDay;
                    Console.SetCursorPosition(Console.WindowWidth - 15, 0);
                    Console.Write(language.TranslationTool("Time:") + $"{DateTime.Now.ToString("HH:mm")}\n");
        ...
        public void AccountMenu(User user, Account account, Language language)
        {
            Console.OutputEncoding = Encoding.UTF8;
            while (true)
            {
                ...
                switch (choice)
                {
                    ...
                    case "2":
                        ExecuteWithdraw(user, account, language);
                        break;

                    case "3":
                        ExecuteTransfer(user, account, language);
                        break;
        ...
        private void ExecuteWithdraw(User user, Account account, Language language)
        {
        ...
        private void ExecuteTransfer(User user, Account account, Language language)
        {
        ...
        /// <returns></returns>
        public Account[] TransferMenu(User user, Language language)
        {
            Account? sender = null;
            ...
            while (true)
            {
                do
                {
                    ...
                    PaintBox(language.TranslationTool("Transfer"), width, height, 9, language);
        ...
        public void PaintBox(string? message, int width, int height, int X, Language language)
        {
